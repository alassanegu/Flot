...
public class MaxFlow implements Algorithm {
	...
	public void init(Graph graph) {
		...
		graph.edges().forEach((Edge e) -> {
			...
			e1.setAttribute("capacite", e.hasNumber(capacityAttribute) ? e.getNumber(capacityAttribute) : Double.POSITIVE_INFINITY);
			e1.setAttribute("flow", 0);
			Edge e2 = network.addEdge(e.getId() + "_rev", e.getTargetNode().getId(), e.getSourceNode().getId(), true);
			e2.setAttribute("capacite", 0);
	...
	public double getCapacity(Edge e) {
		return network.getEdge(e.getId()).getNumber("capacite");
	}
	...
	private void bfs() {
		...
		while (!queue.isEmpty()) {
			Node n = queue.remove();
			n.leavingEdges().forEach((Edge e) -> {
				double cap = e.getNumber("capacite");
	...
	private void augmentFlow() {
		...
		for (Edge e = (Edge) sink.getAttribute("arc"); e != null; e = (Edge) e.getSourceNode().getAttribute("arc")) {
			df = Math.min(df, e.getNumber("capacite") - e.getNumber("flow"));
		}
		totalFlow += df;
		for (Edge e = (Edge) sink.getAttribute("arc"); e != null; e = (Edge) e.getSourceNode().getAttribute("arc")) {
